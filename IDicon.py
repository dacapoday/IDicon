...
class PNGCanvas:
  ...
  def pack_chunk(self,tag,data):#PNG数据块打包
  ...
  def dump(self):
    ...
    for y in range(self.height):
      scanlines.append(0)
      scanlines.extend(self.canvas[(y * self.width * 3):((y+1) * self.width * 3)])    
           #PNG格式头
...
def IDicon(width, height, code, size=10):
    col=hex(hash(code)) 
    color = bytearray([int(col[-9:-7],16),int(col[-13:-11],16),int(col[-11:-9],16)])
